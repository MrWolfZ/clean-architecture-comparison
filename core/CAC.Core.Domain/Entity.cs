...
{
    public abstract record Entity<T, TId>(TId Id) : IEquatable<T>
    ...
    public abstract record EntityId
    {
        protected EntityId(long numericId, Type entityType)
        {
            ...
            if (!entityType.IsAssignableTo(typeof(Entity<,>).MakeGenericType(entityType, GetType())))
            {
                throw new ArgumentException($"type {entityType.Name} must be a subtype of {typeof(Entity<,>).Name}", nameof(entityType));
            }
